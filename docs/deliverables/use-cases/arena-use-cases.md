...
| UC-05 | 토론 생성 | 회원 | 주제와 모드 제출 | 로그인 상태 | ACTIVE 토론 세션 생성 |
...
6. 회원이 토론 주제와 모드를 입력한다.
7. 시스템이 ACTIVE 토론 세션을 생성한다.
8. 회원이 다음 토론 턴을 요청한다.
9. 시스템이 현재 문맥을 Spring AI 연동 계층에 전달한다.
10. AI 연동 계층이 페르소나 발화를 반환한다.
11. 시스템이 발화를 저장한다.
12. 회원이 추가 라운드를 진행하거나 토론을 멈춘다.
13. 시스템이 요약을 저장하고 토론을 STOPPED 상태로 변경한다.
14. 회원이 투표 선택지 A/B를 입력해 중단된 토론을 공유한다.
15. 시스템이 공개 게시글을 생성한다.
16. 비회원 또는 회원이 게시글 목록에서 검색, 필터, 정렬을 사용한다.
17. 회원이 게시글 상세에서 A/B 선택지에 투표한다.
18. 시스템이 선택지별 투표 수와 비율을 갱신한다.
