...
- 토론 생성 화면은 Spring AI 후보 생성 API가 반환한 선택형 후보를 보여줍니다.
...
| 토론 생성 | 원본 주제를 입력하고 Spring AI가 생성한 세부 라운드 후보를 선택하는 화면 |
...
- 현재 `/new` 화면의 후보 생성/검증/정렬은 `POST /api/ai/round-candidates` API를 사용하며, 기존 토론 생성 API 계약은 유지합니다.
